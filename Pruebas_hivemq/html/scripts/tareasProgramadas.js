...
client.on('message', (topic, message) => {
  ...
  if (topic == topic_raiz){
    ...
    if (conex == "Conectado" || conex == "Desconectado") {
      ...
      client.publish(topic_raiz + "/horasProgrmadas", "inicio_horas");
    }
  }
...
function setAlarm() {
  ...
  time_off_mqtt =`0,` + hora_on_3_guardada + `,` + minutos_on_3_guardada +`,` + formato_on_3_guardada + `,` + hora_off_3_guardada + `,` + minutos_off_3_guardada + `,` + formato_off_3_guardada;
    ...
    if (isAlarmSet) {
        ...
        client.publish(topic_raiz + "/hora/recibe/programa/lampara3", time_off_mqtt);
        return (isAlarmSet = false);
      }
      isAlarmSet = true;
      client.publish(topic_raiz + "/hora/recibe/programa/lampara3", time_on_mqtt);
}
